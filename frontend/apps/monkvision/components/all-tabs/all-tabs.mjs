...
function updateFragment(){
  all_tabs.shadowRoot.querySelector('.cardsContainer').innerHTML = pageData.tabs.map((tab, i) =>
      `<div class='flex-item' ix='${i}' onclick='monkshu_env.components['all-tabs'].selectItemHandler(this)'><div>
      ...
      <glowing-arc animate="1" parent="tab" ${tab.title? `innerTitle="${tab.title}"` : ``} ${tab.percentage? `percentage="${tab.percentage}"` : ``} ${tab.color? `color="${tab.color}"` : ``} ${tab.outlineColor? `outlineColor="${tab.outlineColor}"` : ``}></glowing-arc></div>
      </div>
      `
  ).join('');
}
...
function selectItemHandler(el){
  ...
  const prevSelection = $$.querySelector(`.selselectItemHandlerected.flex-item`);
