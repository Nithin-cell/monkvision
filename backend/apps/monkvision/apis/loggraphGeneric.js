...
exports.doService = async jsonReq => {
    ...
    for (let row of rows) {
        ...
        try {
            ...
            if (parsedAddStatus[nodeID][property] == undefined || parsedAddStatus[nodeID][property] == null) throw `Property missing: ${property}`
        } catch (e) { y.push(0); info.push('NA'); continue; }
        eval("y.push((parsedAddStatus[nodeID])."+property+")");
        eval("info.push(\""+property+" - \"+(parsedAddStatus[nodeID])."+property+")");
    }
    const result = {
        result: true,
        type: "linegraph",
