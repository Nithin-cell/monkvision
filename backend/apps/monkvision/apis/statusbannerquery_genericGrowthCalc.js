...
exports.doService = async jsonReq => {
    ...
    if (!(queryParams.$nodeID).split('.')[1]) queryParams.$nodeID = `$.MASTER.${prop_from_pageFile}`
    ...
    const data_array=rows.map(data => data.data_output);
    // Calculation
    // Standard deviation
    ...
    // Data variations
    const [min_num,max_num] = _forLoopMinMax(data_array);
