...
exports.doService = async jsonReq => {
    ...
    for (let row of rows) {
        ...
        for (let i = 0; i < Object.keys(node_array["node_id"]).length; i++) {
            ...
            let Query_nodeHost = node_array["node_info"][keyValData[0]]["hostname"]
            let Query_nodePort = node_array["node_info"][keyValData[0]]["hostPort"]
            let Query_nodeId = keyValData[1];

            x.push(counter);
            y1.push(Query_nodeName);
            y2.push(Query_nodeHost);
            y3.push(Query_nodePort);
            y4.push(Query_nodeId);
