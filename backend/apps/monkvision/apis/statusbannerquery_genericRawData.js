...
exports.doService = async jsonReq => {
    ...
    const multiplicationFactor = jsonReq.multiplicationFactor || 1
    ...
    let truePercent = (Math.fround(rows[0].raw_data).toFixed(2))*multiplicationFactor;
    // calculate issue percentage
    const threshold = jsonReq["threshold"] || 5.00;
    ...
    if ( parseFloat(truePercent) > 1024.00 ){
        truePercent = (parseFloat(truePercent))/1024
        add_symbol = add_symbol.replace("Kb/s", "Mb/s");
    }
