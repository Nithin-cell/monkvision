...
exports.doService = async jsonReq => {
    ...
    if ((Object.keys(JSON.parse(rows[rows.length - 1].additional_status))).includes("total_info")){
        initLength=initLength-1
        LOG.info(`INITLENGTH: ${initLength}, KEYS: ${JSON.stringify(Object.keys(JSON.parse(rows[rows.length - 1].additional_status)))}`);
    }
    ...
    for (let i = 0; i < initLength; i++) legendArray.push(infoArrays[i][infoArrays[i].length-1].split('-')[0].trim().toUpperCase());
