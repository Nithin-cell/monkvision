...
exports.doService = async jsonReq => {
    ...
    const mon_env = jsonReq.mon_env || LOG.error("Monitoring environment not specified.");
    ...
    let x = [],
        ...
        infoArrays = [],
        legendArray = []
    legendArray = (Object.keys(JSON.parse(rows[rows.length - 1].additional_status)))
        .filter(matchingKey => matchingKey.includes(mon_env))
    initLength = legendArray.length;
    ...
    for (let row of rows) {
        ...
        try {
            ...
            for (let arr in yArrays) {
                let data = (Object.keys(parsedAddStatus.total_info).filter(matchingKey => matchingKey.includes(mon_env))[arr]);
...
const _matchByEnvironment = (element, element_env) => {
    if (element.includes(element_env))
        return element
}
